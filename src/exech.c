...
/*
 * output must be dynamically allocated!
 */
          ...
          if (cw + ch_ptr->len >= maxlen)
            {
              maxlen *= 2;
              ERROR(
                  "g_exech_build_string: reallocating output buffer [to %zub]..",
                  maxlen);
              output = realloc(output, maxlen);
              //break;
            }
    ...
    size_t len)
{
  ...
  if (!pp_l || print_ptr[0] == 0xA)
    {
      ERROR("%s: empty print command\n", hdl->file);
      return 2010;
    }
  if (pp_l > len)
    {
      ERROR("%s: print string too large\n", hdl->file);
      return 2004;
    }
  if ((r = g_compile_exech(print_mech, hdl, print_ptr)))
    {
      ERROR("%s: [%d]: could not compile print string\n", hdl->file, r);
      return 2009;
    }

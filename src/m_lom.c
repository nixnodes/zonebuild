          ...
          if (!(_m_ptr->flags & F_GM_LOM_SET))
            {
              if ( _m_ptr->flags & F_GM_ISACCU )
                ...
                {
                  ERROR("%s: invalid LOM match, no F_GM_LOM_SET or F_GM_ISACCU was set\n",
                      hdl->file);
                  return 2;
                }
            }
  ...
  if (loaded > 0)
    {
      ...
      DEBUG("g_load_lom: %s: loaded %d LOM matches\n", hdl->file, loaded);
  ...
  else
    {
      if (0 == rt)
        ...
        {
          DEBUG("g_load_lom: %s: [%d] LOM specified, but none was loaded\n",
              hdl->file, rt);
          return 1;
        }

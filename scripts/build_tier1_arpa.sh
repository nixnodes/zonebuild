...
${ZBUILD} -build inetnum --path ${REGISTRY_PATH}/inetnum --root ${1} --server "${SERVER_NAME_TIER1_ARPA}" --email "${CONTACT_EMAIL}" \
	-lom "nscount > 0 && rfc2317 = 0 && nslevel <= 2  && (pfxsize%8) = 0" \
 	...
 	-preprint "\$TTL ${TIER1_TTL}{:n}{?L:pfxsize >= 24:(?Q:(\{?C:1:startip\}.)):(noop)}{?L:pfxsize >= 16:(?Q:(\{?C:2:startip\}.)):(noop)}{?L:pfxsize >= 8:(?Q:(\{?C:3:startip\}.)):(noop)}in-addr.arpa. {(?P:(?p: )#%-20s)}  {:t} IN {:t}SOA {:t} {server}. {email}. ({curtime} 14400 3600 9600 ${DEFAULT_TTL}){:n}""{?L:pfxsize >= 24:(?Q:(\{?C:1:startip\}.)):(noop)}{?L:pfxsize >= 16:(?Q:(\{?C:2:startip\}.)):(noop)}{?L:pfxsize >= 8:(?Q:(\{?C:3:startip\}.)):(noop)}in-addr.arpa. {(?P:(?p: )#%-20s)} {:t} IN{:t}{:t}TXT{:t}{:t} \"ver=${VERSION},person=${PERSON_HANDLE},rev=`mtn_get_revision ${REGISTRY_PATH}`,ts=`date +%s`\"{:n}" \
